...
const APPROVED_PROJECT_SCOPE =
  'The approved project scope includes a web booking platform, admin dashboard, online payments, appointment management, patient profiles, and notification settings.';

const STORED_CONTRACT_SCOPE =
  'The contract covers the web platform and admin dashboard only. Mobile applications are not included unless approved through a separate written change request.';
...
const RECOMMENDATION_META = {
  approve: {
    ...
    toastMsg: 'Request approved within current scope',
  ...
  reject: {
    ...
    toastMsg: 'Request declined',
    toastType: 'warning' as ToastType,
  },
  convert_to_change_request: {
    label: 'Convert to Change Request',
    icon: 'fa-file-pen',
    className: 'btn-primary',
    toastMsg: 'Change Request created',
    toastType: 'success' as ToastType,
  },
} as const;
...
export default function ScopeGuardPage() {
    ...
    try {
      const res = await fetch('/api/ai/scope', {
        ...
        body: JSON.stringify({
          ...
          existingSrs: compareAgainst === 'srs' ? APPROVED_PROJECT_SCOPE : undefined,
          contractScope: compareAgainst === 'contract' ? STORED_CONTRACT_SCOPE : undefined,
        }),
      });
  ...
  const handleRecommendation = useCallback(
    (rec: 'approve' | 'reject' | 'convert_to_change_request') => {
      setDecision(rec);

      if (rec === 'convert_to_change_request') {
        setShowCRDoc(true);
      } else {
        setShowCRDoc(false);
      }
  ...
  const compareLabel =
    compareAgainst === 'contract'
      ? 'Stored Contract Scope'
      : compareAgainst === 'srs'
      ...
      <div className="page-container animate-fade-in">
        <div className="page-header">
          <div>
            ...
            >
              <p className="page-label" style={{ margin: 0 }}>
                Scope Intelligence
              </p>

              <span className="demo-badge">
                <i className="fa-solid fa-bolt"></i> Gemini AI
              </span>
            ...
            <p className="page-subtitle">
              DocuPilot compares the new client request against stored project scope and stored contract scope, then
              produces a business decision with reason, timeline impact, cost impact, recommended action, and suggested
              client reply.
            </p>
          </div>
        ...
        >
            ...
            <div className="seg-control">
              {(
                [
                  ...
                  ['sow', 'SOW'],
                  ['proposal', 'Proposal'],
                ] as [CompareAgainst, string][]
              ).map(([val, label]) => (
                <button
                  key={val}
                  className={`seg-btn${compareAgainst === val ? ' active' : ''}`}
                  onClick={() => setCompareAgainst(val)}
                >
                  {label}
                </button>
              ))}
            ...
            <div className="seg-control">
              {(
                [
                  ...
                  ['english', 'English'],
                  ['arabic', 'Arabic'],
                ] as [AnalysisLang, string][]
              ).map(([val, label]) => (
                <button
                  key={val}
                  className={`seg-btn${analysisLang === val ? ' active' : ''}`}
                  onClick={() => setAnalysisLang(val)}
                >
                  {label}
                </button>
              ))}
            ...
            <div className="seg-control">
              {(
                [
                  ...
                  ['balanced', 'Balanced'],
                  ['lenient', 'Lenient'],
                ] as [Strictness, string][]
              ).map(([val, label]) => (
                <button
                  key={val}
                  className={`seg-btn${strictness === val ? ' active' : ''}`}
                  onClick={() => setStrictness(val)}
                >
                  {label}
                </button>
              ))}
          ...
          <div style={{ marginLeft: 'auto' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              ></span>
              <span style={{ fontSize: '0.875rem', fontWeight: 700, color: statusMeta.color }}>
                {statusMeta.label}
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 section-mb">
          <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <textarea
              ...
              style={{
                ...
                fontSize: '1rem',
                lineHeight: 1.8,
                textAlign: /[\u0600-\u06FF]/.test(requestText) ? 'right' : 'left',
              }}
            ...
            {analysis ? (
              <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: 'var(--spacing-md)', flex: 1 }}>
                <div className="text-xs text-muted font-semibold uppercase tracking-wider" style={{ marginBottom: '6px' }}>
                  Strategic Impact
                </div>
          ...
          <div
            className="card"
            style={{ padding: 0, display: 'flex', flexDirection: 'column', borderColor: statusMeta.border }}
          >
            <div
              className="scope-result-header"
              style={{ background: statusMeta.bg, borderBottom: `1px solid ${statusMeta.border}` }}
            >
              <div style={{ display: 'flex', gap: 'var(--spacing-md)', alignItems: 'center' }}>
                ...
                <div>
                  <h3 className="font-bold" style={{ fontSize: '1.0625rem', color: statusMeta.color }}>
                    Analysis Result
                  </h3>
            ...
            >
              <div>
                <div className="text-xs text-muted font-semibold uppercase tracking-wider" style={{ marginBottom: 'var(--spacing-sm)' }}>
                  Primary Reason
                </div>
                ...
                >
                  <i
                    ...
                    style={{ color: statusMeta.color, marginTop: '3px', flexShrink: 0 }}
                  ></i>
              ...
              <div className="grid grid-cols-2" style={{ gap: 'var(--spacing-md)' }}>
                {impacts.map((imp) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className="text-xs text-muted font-semibold uppercase tracking-wider"
                      ...
                      >
                        <div
                          style={{
                            height: '100%',
                            width: imp.level === 'low' ? '33%' : imp.level === 'medium' ? '66%' : '100%',
              ...
              >
                <div>
                  <div className="text-xs text-muted font-semibold uppercase tracking-wider" style={{ marginBottom: '4px' }}>
                    AI Confidence Score
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px' }}>
                    <span
                      style={{
                        ...
                        fontWeight: 800,
                        fontFamily: 'var(--font-display)',
                        color: (analysis?.confidenceScore ?? 87) >= 80 ? 'var(--status-success)' : 'var(--accent-primary)',
                      }}
                    >
                ...
                <div style={{ flex: 1, maxWidth: '120px' }}>
                  <div className="progress-container" style={{ height: '8px' }}>
                    <div
                      ...
                      style={{
                        width: `${analysis?.confidenceScore ?? 87}%`,
                        background: (analysis?.confidenceScore ?? 87) >= 80 ? 'var(--status-success)' : 'var(--accent-primary)',
                      }}
                    ></div>
                  </div>
                ...
                >
                  <div style={{ display: 'flex', gap: 'var(--spacing-md)', alignItems: 'flex-start' }}>
                    ...
                    <div>
                      ...
                      <div className="text-sm" style={{ marginTop: '2px', lineHeight: 1.5 }}>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          ...(isApprove && isChosen
                            ? { background: 'var(--status-success)', borderColor: 'var(--status-success)' }
                            : {}),
                          ...(isReject && isChosen
                            ? { background: 'var(--status-danger)', borderColor: 'var(--status-danger)', color: 'white' }
                            : {}),
                        }}
                      >
            ...
            <div style={{ paddingTop: 'var(--spacing-md)' }}>
              ...
              <div className="cr-document">
                <div className="cr-document-row">
                  ...
                  <span className="cr-document-val">
                    {analysis?.changeRequestSummary ?? 'Addition of a mobile app to the Clinic Booking Platform scope.'}
                  </span>
                </div>
                ...
                <div className="cr-document-row">
                  ...
                  <span className="cr-document-val">
                    {analysis?.reason ??
                      `Client requested cross-platform mobile presence to reach users on the go. This is not covered in ${compareLabel}.`}
                  </span>
                </div>
                ...
                <div className="cr-document-row">
                  ...
                  <span
                    className="cr-document-val"
                    style={{ color: IMPACT_META[analysis?.timelineImpact ?? 'high'].color, fontWeight: 600 }}
                  >
                ...
                <div className="cr-document-row">
                  ...
                  <span
                    className="cr-document-val"
                    style={{ color: IMPACT_META[analysis?.costImpact ?? 'high'].color, fontWeight: 600 }}
                  >
                ...
                <div className="cr-document-row">
                  ...
                  <span
                    className="cr-document-val"
                    style={{ color: IMPACT_META[analysis?.businessImpact ?? 'medium'].color, fontWeight: 600 }}
                  >
                    {IMPACT_META[analysis?.businessImpact ?? 'medium'].label} impact
                  </span>
                </div>
                ...
                <div className="cr-document-row">
                  ...
                  <span
                    className="cr-document-val"
                    style={{ color: IMPACT_META[analysis?.riskImpact ?? 'high'].color, fontWeight: 600 }}
                  >
                    {IMPACT_META[analysis?.riskImpact ?? 'high'].label} impact
                  </span>
                </div>
                ...
                <div className="cr-document-row">
                  ...
                  <div style={{ display: 'flex', gap: 'var(--spacing-md)', alignItems: 'center' }}>
                    <div style={{ display: 'flex' }}>
                      {[
                        { initials: 'AD', bg: 'var(--accent-primary)' },
                        { initials: 'PM', bg: 'var(--status-info)' },
                      ].map((a, i) => (
                        <div
                          key={i}
                          className="avatar"
                  ...
                  <div className="grid-2col">
                    {['Client Signature', 'PM Signature'].map((sig) => (
                      <div key={sig} style={{ borderBottom: '1px dashed var(--border-strong)', paddingBottom: '4px' }}>
                        <div className="text-xs text-muted font-semibold uppercase tracking-wider" style={{ marginTop: '24px' }}>
                          {sig}
                        </div>
                      </div>
                    ))}
            ...
            <div className="card-header" style={{ borderBottom: '1px solid var(--border-subtle)', paddingBottom: 'var(--spacing-md)' }}>
              ...
              <div style={{ display: 'flex', gap: 'var(--spacing-sm)' }}>
                <button
                  ...
                  onClick={() => {
                    if (analysis) {
                      navigator.clipboard.writeText(analysis.clientReply).catch(() => undefined);
                    }
        ...
        <div className="grid grid-cols-3">
          <div className="card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-sm)' }}>
              <div className="data-row">
                <span className="data-label">Agreement</span>
                <span className="data-value font-bold">Al Waha Clinics Service Agreement</span>
              </div>

              <div className="data-row">
                <span className="data-label">Client</span>
                <span className="data-value font-bold">Al Waha Clinics</span>
              </div>

              <div>
                <div className="data-row" style={{ borderBottom: 'none', paddingBottom: 'var(--spacing-xs)' }}>
                  <span className="data-label">Project Scope Coverage</span>
                  <span className="data-value font-bold">Web + Admin</span>
                </div>
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-sm)' }}>
              {[
                { label: 'Notification Settings', status: 'In Scope', type: 'success' },
                { label: 'Mobile App Request', status: 'Out of Scope', type: 'danger' },
              ].map((item) => (
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)', marginTop: 'var(--spacing-sm)' }}>
              <div style={{ display: 'flex' }}>
                {[
                  { initials: 'AD', z: 3, bg: 'var(--accent-primary)' },
                  { initials: 'PM', z: 2, bg: 'var(--status-info)' },
                  { initials: '+4', z: 1, bg: 'var(--bg-surface-elevated)', color: 'var(--text-muted)' },
                ].map((a, i) => (
                  <div
                    key={i}
                    className="avatar"
